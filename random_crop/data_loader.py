...
class image_loader(Dataset):
    def __init__(self, transforms_=None, **args):
        ...
        self.files = self.files[:-val_split] if self.mode == 'train' else self.files[-val_split:]
        self.transform = transforms.Compose(transforms_) if transforms_ else None
    
    def random_crop(self, img):
        x1, y1 = random.randint(0, self.img_x-self.mask_x), random.randint(0, self.img_y-self.mask_y)
